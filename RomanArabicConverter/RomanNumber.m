...
- (id) initWithString: (NSString *)string {
    [self initArrays];
    _romanString = string;
    NSAssert([self intValue] > 0 && [self intValue] < 3999, @"Number outside of supported roman bounds");
    return self;
}

- (id)initWithValue:(NSUInteger)value {
    NSAssert(value > 0 && value < 3999, @"Number outside of supported roman bounds");
    [self initArrays];
    NSString* roman = @"";
    NSUInteger n = value;
...
- (void)initArrays { //technically it is possible to support numbers over 4000 when using additional digits or allowing the use of more than 3 consecutive letters M
